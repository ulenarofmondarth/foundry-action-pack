...
Hooks.on('init', () => {
  game.settings.register('action-pack', 'tray-display', {
  ...
  game.settings.register('action-pack', 'assume-default-character', {
  ...
  game.settings.register('action-pack', 'icon-size', {
  ...
  game.settings.register('action-pack', 'tray-size', {
  ...
  game.settings.register('action-pack', 'skill-mode', {
  ...
  game.settings.register('action-pack', 'show-spell-dots', {
  ...
  game.settings.register('action-pack', 'show-no-uses', {
  ...
  game.settings.register('action-pack', 'sort-alphabetic', {
  ...
  game.settings.register('action-pack', 'show-unprepared-cantrips', {
  ...
  game.settings.register('action-pack', 'use-control-button', {
...
async function updateTray() {
  ...
  const actors = getActiveActors().map(actor => {
    ...
    function removeEmptySections(sections) {
      function hasItems(object) {
        ...
        if (keys.includes('groups') && Object.values(object.groups).some(g => hasItems(g))) {
          return true;
        }
  ...
  const htmlString = await renderTemplate('modules/action-pack/templates/action-pack.hbs', {
    actors,
    iconSize,
    abilities,
    showSpellDots,
  ...
  function roll(event) {
    ...
    if (item) {
      if (
        !game.modules.get('wire')?.active &&
        game.modules.get('itemacro')?.active &&
        game.settings.get('itemacro', 'defaultmacro')
      ) {
        if (item.hasMacro()) {
          item.executeMacro();
          return false;
